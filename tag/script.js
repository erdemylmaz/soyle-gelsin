...
if(localStorage.getItem('tag') != '') {
  ...
  tag.products.map((product) => {
    if(product.tag.toLowerCase() == tag.tag.toLowerCase()) {
      tag.taggedProducts.push(product);
    }
  });

  tag.taggedProducts.map((product) => {
  ...
  products.forEach((product) => {
    product.addEventListener('click', tag.addToCart);
  });


  document.title = `${tag.tag.toUpperCase()} | Soyle Gelsin`;
} else {
  productsArea.innerHTML = `
      <div class="emptyPage">
        <h3>Please select one tag from header bar</h3>
      </div>
